import { ExamData } from "../examData";

const pte: ExamData = {
  name: "PTE",
  bannerImage: "/test-prep/pte-banner1.webp",
  bannerSubtitle:
    "Achieve fast and reliable English test results for university entry and visa applications with PTE. Experience flexible testing dates and quick score reports.",
  description:
    "The Pearson Test of English (PTE) is recognized globally for academic and immigration purposes. Skill Up's expert guidance helps you tackle all test modules with confidence, providing practice in speaking, writing, reading, and listening, while focusing on test-taking strategies for fast and accurate results",
  preparation: [
    {
      id: "01",
      title: "Individual Focus",
    },
    {
      id: "02",
      title: "Customized Programs",
    },
    {
      id: "03",
      title: "Exhaustive Study Materials",
    },
    {
      id: "04",
      title: "Practice Tests",
    },
    {
      id: "05",
      title: "Online Classes",
    },
    {
      id: "06",
      title: "Solving Doubts",
    },
  ],
  aboutImage:"/test-prep/pte1.webp",
  tableTitle: "Pearson Test of English (PTE)",
  tableData: [
      { title: "Testing authority", values: "Pearsons, Australia" },
      { title: "Registration website", values: "https://www.pearsonpte.com/" },
      { title: "Sections", values: "Reading | Writing | Listening | Speaking" },
      { title: "Applicability", values: "Overseas Education & Employment" },
      { title: "Acceptability", values: "UK, USA, Canada, Australia, NZ, EU" },
      { title: "Eligibility", values: "Std. 10+" },
      { title: "Duration", values: "2 hours 45 minutes" },
      { title: "Scoring", values: "Minimum 65 out of 90" },
      { title: "Modes", values: "Computer Based" },
      { title: "Validity", values: "2 years" },
      { title: "Identity document", values: "Passport" },
      { title: "Approximate test fee", values: "Rs.17,000 (As on 2024)" },
      {
        title: "Re-attempt/schedule/cancel",
        values: "Possible | Refer to web-site",
      },
  ],
  training: [
    { title: "Training mode", values: "Off-line" },
    { title: "Course materials", values: "Printed copies included" },
    { title: "Training", values: "30 hours" },
    { title: "Self-study", values: "30 hours" },
    { title: "Mock-tests", values: "15 hours (5 mock-tests)" },
    { title: "Total training program", values: "75 hours" },
  ],
  note: "All information mentioned here-in are indicative. Do refer to the web-sites of the relevant test for current information.",
};

export default pte;
