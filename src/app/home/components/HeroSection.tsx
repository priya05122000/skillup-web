...
const HeroSection: React.FC = () => {
  return (
    <section
      className="relative h-[calc(90vh-80px)] w-full mb-20"
      aria-label="Hero"
    >
      ...
      <div className="absolute max-w-2xl z-50 bg-(--white)/10 bottom-0 left-1/2 -translate-x-1/2 translate-y-1/2 h-auto w-full px-8 py-8 rounded-md border border-(--white)/10">
        <div className="flex justify-evenly z-50">
          <HeroButton icon={<MdArrowOutward />}>Apply Now</HeroButton>
          <HeroButton icon={<MdOutlineWhatsapp />}>Chat with Us</HeroButton>
          <HeroButton icon={<MdArrowOutward />}>Brochure</HeroButton>
        </div>
      </div>
    </section>
  );
